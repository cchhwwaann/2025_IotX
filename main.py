# -*- coding: utf-8 -*-
# 얼굴 추적 (서보 제어)과 손 추적 (웹 서버 제어)을 모두 포함하는 통합 스크립트입니다.
# 웹 페이지는 별도의 HTML/JS 파일이 처리합니다.
...
def run_combined_tracking_and_control():
    ...
    try:
        while True:
            ...
            if arduino and face_results.detections:
                for detection in face_results.detections:
                    bboxC = detection.location_data.relative_bounding_box
                    face_center_x = bboxC.xmin + bboxC.width / 2
                    
                    # 서보 모터를 움직일 방향 결정 (간결한 로직)
                    direction = 0
                    if face_center_x < 0.4:  # 얼굴이 화면 좌측에 있을 경우
                        direction = 1  # 시계 방향
                    elif face_center_x > 0.6: # 얼굴이 화면 우측에 있을 경우
                        direction = -1 # 반시계 방향
                    
                    # 아두이노로 방향 값 전송
                    arduino.write(f"{direction}\n".encode())
                    print(f"얼굴 중심: {face_center_x:.2f}, 전송된 방향: {direction}")

                    # 시각화를 위한 원 그리기
                    center_point_x = int(face_center_x * w)
                    center_point_y = int(bboxC.ymin * h + bboxC.height * h / 2)
                    cv2.circle(image, (center_point_x, center_point_y), 10, (255, 0, 0), -1)
